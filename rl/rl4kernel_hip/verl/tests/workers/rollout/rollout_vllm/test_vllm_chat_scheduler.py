...
def test_vllm_async_rollout_without_tool_calls(init_config):
    ...
    runtime = ray.get(async_rollout_manager.async_llm_servers[0].describe_runtime.remote())
    assert runtime["engine_initialized"] is True
    assert runtime["server_ready"] is True
    assert runtime["cuda_visible_devices"] not in (None, "", "not set")
