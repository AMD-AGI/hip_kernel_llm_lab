...
def _expected_kde_advantages(token_level_rewards, response_mask, eps_l, eps_u, tau, alpha, beta):
    scores = token_level_rewards.sum(dim=-1)
    std = torch.std(scores, unbiased=False)
    bandwidth = 1.06 * std * (scores.numel() ** (-0.2)) + 1e-6
    cdf = verl.trainer.ppo.core_algos._gaussian_kde_cdf(scores, bandwidth)
    cdf = torch.clamp(cdf, eps_l, 1.0 - eps_u)
    utility = alpha * torch.log(cdf) - beta * torch.log1p(-cdf)
    adv_per_sample = utility / max(float(tau), 1e-8)
    adv_per_sample = adv_per_sample - adv_per_sample.mean()
    return adv_per_sample.unsqueeze(-1).tile((1, token_level_rewards.shape[-1])) * response_mask


def test_kde_logodds_alpha_beta_default_matches_legacy_logit():
    token_level_rewards = torch.tensor([[0.0, 0.0], [0.4, 0.6], [0.9, 1.1]], dtype=torch.float64)
    response_mask = torch.tensor([[1.0, 1.0], [1.0, 0.0], [1.0, 1.0]], dtype=torch.float64)
    index = np.array(["group-0", "group-0", "group-0"], dtype=object)

    advantages, returns = verl.trainer.ppo.core_algos.compute_kde_logodds_outcome_advantage(
    ...
    expected = _expected_kde_advantages(
        token_level_rewards=token_level_rewards,
        response_mask=response_mask,
        eps_l=0.01,
        eps_u=0.01,
...
def test_kde_logodds_alpha_beta_strengthens_left_tail_penalty():
    token_level_rewards = torch.tensor([[0.0, 0.0], [0.4, 0.6], [0.9, 1.1]], dtype=torch.float64)
    response_mask = torch.ones_like(token_level_rewards)
    index = np.array(["group-0", "group-0", "group-0"], dtype=object)

    default_advantages, _ = verl.trainer.ppo.core_algos.compute_kde_logodds_outcome_advantage(
        token_level_rewards=token_level_rewards,
        response_mask=response_mask,
        index=index,
        alpha=1.0,
    ...
    skewed_advantages, _ = verl.trainer.ppo.core_algos.compute_kde_logodds_outcome_advantage(
        token_level_rewards=token_level_rewards,
        response_mask=response_mask,
        index=index,
        alpha=2.0,
    ...
    expected_skewed = _expected_kde_advantages(
        token_level_rewards=token_level_rewards,
        response_mask=response_mask,
        eps_l=0.01,
        eps_u=0.01,
    ...
    torch.testing.assert_close(skewed_advantages, expected_skewed)

    default_per_sample = default_advantages[:, 0]
    skewed_per_sample = skewed_advantages[:, 0]
    assert skewed_per_sample[0] < default_per_sample[0]
    assert torch.isclose(skewed_per_sample.mean(), torch.tensor(0.0, dtype=skewed_per_sample.dtype))
