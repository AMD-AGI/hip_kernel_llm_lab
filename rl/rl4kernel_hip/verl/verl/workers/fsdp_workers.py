...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        # These flags let us split init_model() into pre-checkpoint and post-checkpoint
        # phases without rebuilding modules. This keeps resume semantics unchanged while
        # avoiding unnecessary rollout/ref memory before actor checkpoint restore starts.
        self._actor_rollout_backbone_initialized = False
        self._actor_runtime_initialized = False
        self._rollout_initialized = False
        self._ref_initialized = False
        self._checkpoint_manager_initialized = False
    ...
    def _ensure_actor_rollout_backbone(
        self,
        override_model_config,
        use_remove_padding,
        use_shm,
        ...
        if self._actor_rollout_backbone_initialized or not (self._is_actor or self._is_rollout):
            return

        # The actor FSDP module and optimizer must exist before checkpoint load because
        # strict resume restores actor shards directly into these live objects.
        if self._is_actor:
        ...
        self._actor_rollout_backbone_initialized = True

    def _ensure_actor_runtime(self, DataParallelPPOActor, use_remove_padding, use_fused_kernels):
        if self._actor_runtime_initialized or not self._is_actor:
            return
        ...
        self._actor_runtime_initialized = True

    def _ensure_rollout_runtime(self, trust_remote_code=False):
        if self._rollout_initialized or not self._is_rollout:
            return

        assert self._actor_rollout_backbone_initialized, "actor/rollout backbone must be initialized before rollout"
    ...
    def _ensure_ref_runtime(
        ...
        if self._ref_initialized or not self._is_ref:
            return
    ...
    def _ensure_checkpoint_manager(self):
        if self._checkpoint_manager_initialized:
            return
        ...
        if self._is_actor:
            ...
            # Standalone rollout still needs a model-only checkpoint manager so it can
            # load FSDP actor checkpoints without constructing actor optimizer state.
        ...
        self._checkpoint_manager_initialized = True

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def init_model(self, build_rollout: bool = True):
        ...
        self._ensure_actor_rollout_backbone(
            override_model_config=override_model_config,
            use_remove_padding=use_remove_padding,
            use_shm=use_shm,
            use_fused_kernels=use_fused_kernels,
        )
        self._ensure_actor_runtime(
            DataParallelPPOActor=DataParallelPPOActor,
            use_remove_padding=use_remove_padding,
            use_fused_kernels=use_fused_kernels,
        )
        self._ensure_checkpoint_manager()

        # rollout is intentionally optional here: for FSDP strict resume we first build
        # the actor-side checkpoint targets, restore the actor checkpoint, and only then
        # materialize rollout so its engine memory does not inflate the pre-load baseline.
        if build_rollout:
            self._ensure_rollout_runtime(trust_remote_code=self.config.model.get("trust_remote_code", False))

        self._ensure_ref_runtime(
            DataParallelPPOActor=DataParallelPPOActor,
            override_model_config=override_model_config,
            use_remove_padding=use_remove_padding,
            use_shm=use_shm,
...
class AsyncActorRolloutRefWorker(ActorRolloutRefWorker):
    ...
    async def execute_method(self, method: Union[str, bytes], *args, **kwargs):
        """Called by ExternalRayDistributedExecutor collective_rpc."""
        if self.vllm_tp_rank == 0 and method != "execute_model":
            print(f"[DP={self.vllm_dp_rank},TP={self.vllm_tp_rank}] execute_method: {method if isinstance(method, str) else 'Callable'}", flush=True)
