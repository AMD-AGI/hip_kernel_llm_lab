...
class CompletionCallback(ABC):
    def __init__(self, config: DictConfig, scheduler: "ChatCompletionScheduler"):
        ...
        self.enable_thinking = config.actor_rollout_ref.rollout.multi_turn.get("enable_thinking", True)
    ...
    def postprocess(self, batch: DataProto, batch_conversations: List[List[Dict[str, str]]], n: int) -> DataProto:
        ...
        Returns:
            Batch data, should include ["prompts", "responses", "response_mask", "loss_mask", "input_ids", "attention_mask", "position_ids"].
        """
        raise NotImplementedError

    async def on_request_complete(self, info: Dict[str, Any]) -> None:
        """Optional hook for cleanup after a logical rollout request finishes."""
        return None

    def _apply_chat_template(self, messages: List[Dict[str, str]], add_generation_prompt: bool, tokenize: bool) -> str:
        kwargs = {
...
class ToolCompletionCallback(CompletionCallback):
    def __init__(self, config: DictConfig, scheduler: "ChatCompletionScheduler"):
        ...
        self.max_tool_calls = config.actor_rollout_ref.rollout.multi_turn.get("max_tool_calls", None)
        self.max_tool_wallclock_s = config.actor_rollout_ref.rollout.multi_turn.get("max_tool_wallclock_s", None)
    ...
    async def __call__(self, messages: List[Dict[str, str]], completions: ChatCompletion, info: Dict[str, Any]):
        ...
        limit_reason = self._check_tool_limits(info, requested_calls=len(tool_calls))
        if limit_reason is not None:
            print(f"[id={completions.id},turn={len(messages)},finish_reason={finish_reason}] {limit_reason}")
            return
        print(f"[id={completions.id},turn={len(messages)},finish_reason={finish_reason}] Call {len(tool_calls)} tools")
        tasks = []
        for tool_call in tool_calls:
            tasks.append(self._call_tool(tool_call, info))
    ...
    def _request_key(self, info: Dict[str, Any]) -> str:
        return info["__request_key__"]

    def _request_tools_kwargs(self, info: Dict[str, Any]) -> Dict[str, Any]:
        return info.get("__tools_kwargs__", {}) or {}

    def _check_tool_limits(self, info: Dict[str, Any], requested_calls: int) -> str | None:
        if self.max_tool_calls is not None:
            current_calls = int(info.get("__tool_calls__", 0))
            if current_calls + requested_calls > int(self.max_tool_calls):
                return f"Tool call budget exceeded ({current_calls + requested_calls}>{self.max_tool_calls}), terminate request."
        if self.max_tool_wallclock_s is not None:
            started_at = float(info.get("__request_started_at__", time.time()))
            elapsed_s = time.time() - started_at
            if elapsed_s > float(self.max_tool_wallclock_s):
                return f"Tool wallclock budget exceeded ({elapsed_s:.2f}s>{self.max_tool_wallclock_s}s), terminate request."
        return None

    async def _call_tool(self, tool_call, info: Dict[str, Any]) -> Dict[str, str]:
        """Call tool and return tool response."""
        tool_name = tool_call.function.name
        tool_args = json.loads(tool_call.function.arguments)
        tool = self.tools[tool_name]
        request_key = self._request_key(info)
        request_state = self._request_state.setdefault(request_key, {"created_tools": set()})
        tool_kwargs = self._request_tools_kwargs(info).get(tool_name, {})

        if tool_name not in request_state["created_tools"]:
            create_kwargs = tool_kwargs.get("create_kwargs", {})
            await tool.create(request_key, **create_kwargs)
            request_state["created_tools"].add(tool_name)
        try:
            execute_kwargs = tool_kwargs.get("execute_kwargs", {})
            info["__tool_calls__"] = int(info.get("__tool_calls__", 0)) + 1
            tool_response, tool_reward_score, tool_metrics = await tool.execute(request_key, tool_args, **execute_kwargs)
    ...
    async def on_request_complete(self, info: Dict[str, Any]) -> None:
        request_key = info.get("__request_key__")
        if not request_key:
            return
        request_state = self._request_state.pop(request_key, None)
        if not request_state:
            return

        tools_kwargs = self._request_tools_kwargs(info)
        for tool_name in request_state["created_tools"]:
            tool = self.tools[tool_name]
            release_kwargs = tools_kwargs.get(tool_name, {}).get("release_kwargs", {})
            try:
                await tool.release(request_key, **release_kwargs)
            except Exception as e:
                logger.exception(f"Error when releasing tool {tool_name}: {e}")
    ...
    def postprocess(self, batch: DataProto, batch_conversations: List[List[Dict[str, str]]], n: int) -> DataProto:
        ...
        prompts = [self._apply_chat_template(prompt, add_generation_prompt=True, tokenize=False) for prompt in batch.non_tensor_batch["raw_prompt"]]
        assert len(batch_conversations) == len(prompts) * n

        # sequences: [prompt + response]
        sequences = [self._apply_chat_template(conversation, add_generation_prompt=False, tokenize=False) for conversation in batch_conversations]
        ...
        # response_mask/loss_mask: mask out tool-call spans inside the response.
        # Prompt tokens always have loss_mask=0, matching the trainer contract used by
        # the multi-turn GRPO path and the rollout schemas from other backends.
        ...
        prompt_loss_mask = torch.zeros_like(prompts["attention_mask"])
...
class ChatCompletionScheduler:
    ...
    async def _submit_chat_completions_and_callback(
        ...
        if info["__depth__"] == 0:
            await self.completion_callback.on_request_complete(info)
            info["__done__"].set()
    ...
    async def generate_sequences(self, batch: DataProto) -> DataProto:
        ...
        repeated_raw_prompts = batch.non_tensor_batch["raw_prompt"].repeat(n, axis=0)
        repeated_tools_kwargs = None
        if "tools_kwargs" in batch.non_tensor_batch:
            repeated_tools_kwargs = batch.non_tensor_batch["tools_kwargs"].repeat(n, axis=0)

        for batch_index, conversation in enumerate(repeated_raw_prompts):
            # raw_prompt: [{"role": "user", "content": ""}, ["role": "assistant", "content"], ...]
            batch_conversations[batch_index] = conversation.tolist()
            raw_tools_kwargs = {} if repeated_tools_kwargs is None else repeated_tools_kwargs[batch_index]
            if isinstance(raw_tools_kwargs, dict):
                request_tools_kwargs = raw_tools_kwargs
            elif hasattr(raw_tools_kwargs, "item"):
                raw_item = raw_tools_kwargs.item()
                request_tools_kwargs = raw_item if isinstance(raw_item, dict) else {}
            else:
                request_tools_kwargs = {}
            ...
            tasks.append(
                asyncio.create_task(
                    self._submit_chat_completions_semaphore(
                        ...
                        tools_kwargs=request_tools_kwargs,
                    )
                )
            )
    ...
    async def _submit_chat_completions_semaphore(
        ...
        info = {
            ...
            "__request_key__": uuid4().hex,
            "__request_started_at__": time.time(),
            "__tool_calls__": 0,
            "__tools_kwargs__": tools_kwargs or {},
        }
