        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    pg_loss_agg_mode = self.config.policy_loss.get("loss_agg_mode", "seq-mean-token-sum-norm")
                    ...
                    advantage_shaping = self.config.advantage_shaping
                    if advantage_shaping == "add":
                        alpha = 0.4; kappa = 2
                    ...
                    if loss_mode == "vanilla":
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = compute_policy_loss(
                            ...
                            loss_agg_mode=pg_loss_agg_mode,
                        )
                    else:
                        policy_loss_fn = get_policy_loss_fn(loss_mode)
                        pg_loss, pg_clipfrac, ppo_kl, pg_clipfrac_lower = policy_loss_fn(
                            old_log_prob, log_prob, advantages, response_mask, pg_loss_agg_mode, self.config
                        )
