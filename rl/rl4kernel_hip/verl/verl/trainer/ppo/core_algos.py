...
def _gaussian_kde_cdf(x: torch.Tensor, bandwidth: torch.Tensor):
    """Estimate CDF for each sample with Gaussian KDE."""
    if x.numel() == 1:
        return torch.full_like(x, 0.5)

    bw = torch.as_tensor(bandwidth, dtype=x.dtype, device=x.device).clamp_min(1e-6)
    eval_points = x.unsqueeze(-1)  # (k, 1)
    centers = x.unsqueeze(0)  # (1, k)

    z = (eval_points - centers) / bw
    cdf = 0.5 * (1.0 + torch.erf(z / np.sqrt(2.0)))
    return cdf.mean(dim=-1)


@register_adv_est(AdvantageEstimator.KDE_LOGODDS)
def compute_kde_logodds_outcome_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    index: np.ndarray,
    eps_l: float = 0.01,
    ...
    **kwargs,
    ...
    """
    Compute KDE percentile-utility advantage on outcome reward (one scalar per response).

    The utility is:
        alpha * log(u) - beta * log(1 - u)
    where u is the KDE-estimated CDF. The legacy log-odds behavior is recovered with
    alpha=1 and beta=1.

    Args:
        token_level_rewards: `(torch.Tensor)` shape: (bs, response_length)
        response_mask: `(torch.Tensor)` shape: (bs, response_length)
        index: `(np.ndarray)` prompt uid for grouping
        eps_l: lower CDF margin, clamp lower bound is eps_l
        eps_u: upper CDF margin, clamp upper bound is (1 - eps_u)
        tau: temperature for utility scaling
        alpha: left-tail utility weight, clamped to be non-negative
        beta: right-tail utility weight, clamped to be non-negative

    Returns:
        advantages: `(torch.Tensor)` shape: (bs, response_length)
        returns: `(torch.Tensor)` shape: (bs, response_length)
    """
    # Boundary robustness: keep (0, 1) and ensure eps_l < (1 - eps_u) to avoid NaN/explosion
    eps_l = max(1e-6, min(eps_l, 1.0 - 1e-6))
    eps_u = max(1e-6, min(eps_u, 1.0 - 1e-6))
    if eps_l >= 1.0 - eps_u:
        eps_u = max(1e-6, 1.0 - eps_l - 1e-4)
    alpha = max(0.0, float(alpha))
    beta = max(0.0, float(beta))

    scores = token_level_rewards.sum(dim=-1)
    response_length = token_level_rewards.shape[-1]
    adv_per_sample = torch.zeros_like(scores)
    ...
    with torch.no_grad():
        ...
        for _, positions in id2pos.items():
            group_scores = scores[positions]
            group_scores = group_scores.detach()
            k = group_scores.numel()
            if k <= 1:
                adv = torch.zeros_like(group_scores)
            else:
                std = torch.std(group_scores, unbiased=False)
                bandwidth = 1.06 * std * (k ** (-0.2)) + 1e-6
                cdf = _gaussian_kde_cdf(group_scores, bandwidth)
                cdf = torch.clamp(cdf, eps_l, 1.0 - eps_u)
                utility = alpha * torch.log(cdf) - beta * torch.log1p(-cdf)
                adv = utility
                adv = adv / max(float(tau), 1e-8)
                adv = adv - adv.mean()

            adv_per_sample[positions] = adv

        adv_per_token = adv_per_sample.unsqueeze(-1).tile([1, response_length]) * response_mask

    return adv_per_token, adv_per_token
