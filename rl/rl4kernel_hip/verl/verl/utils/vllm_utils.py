...
_SUPPORTED_MOE_MODELS: tuple[type, ...] | None = None
_MIXTRAL_MODEL_CLS: type | None = None
_TENSOR_LORA_REQUEST_CLS: type | None = None


def _import_optional_attr(module_path: str, attr_name: str) -> Any | None:
    """Import optional vLLM symbols lazily.

    Some vLLM model modules inspect GPU state during import on ROCm. CPU-only Ray
    actors only need lightweight helpers such as `is_version_ge`, so avoid eager
    imports that would fail before any GPU worker is created.
    """
...
def _get_supported_moe_models() -> tuple[type, ...]:
    global _SUPPORTED_MOE_MODELS
    if _SUPPORTED_MOE_MODELS is None:
        models = []
        for module_path, attr_names in (
            ("vllm.model_executor.models.deepseek_v2", ("DeepseekV2ForCausalLM", "DeepseekV3ForCausalLM")),
            ...
            for attr_name in attr_names:
                model_cls = _import_optional_attr(module_path, attr_name)
                if model_cls is not None:
                    models.append(model_cls)
        _SUPPORTED_MOE_MODELS = tuple(models)
    return _SUPPORTED_MOE_MODELS


def _get_mixtral_model_cls() -> type | None:
    global _MIXTRAL_MODEL_CLS
    if _MIXTRAL_MODEL_CLS is None:
        _MIXTRAL_MODEL_CLS = _import_optional_attr("vllm.model_executor.models.mixtral", "MixtralForCausalLM")
    return _MIXTRAL_MODEL_CLS


def _get_lora_runtime():
    from vllm.lora.models import LoRAModel
    from vllm.lora.request import LoRARequest
    from vllm.lora.utils import get_adapter_absolute_path
    from vllm.lora.worker_manager import LRUCacheWorkerLoRAManager

    return LoRAModel, LoRARequest, get_adapter_absolute_path, LRUCacheWorkerLoRAManager


def _get_tensor_lora_request_cls() -> type:
    global _TENSOR_LORA_REQUEST_CLS
    if _TENSOR_LORA_REQUEST_CLS is None:
        _, LoRARequest, _, _ = _get_lora_runtime()

        class _TensorLoRARequest(LoRARequest):
            peft_config: dict = field(default=None)
            lora_tensors: dict = field(default=None)

        _TensorLoRARequest.__name__ = "TensorLoRARequest"
        _TENSOR_LORA_REQUEST_CLS = _TensorLoRARequest
    return _TENSOR_LORA_REQUEST_CLS


def _is_tensor_lora_request(value: Any) -> bool:
    return hasattr(value, "peft_config") and hasattr(value, "lora_tensors")


def TensorLoRARequest(*args, **kwargs):
    return _get_tensor_lora_request_cls()(*args, **kwargs)
...
def patch_vllm_moe_model_weight_loader(model):
    ...
    supported_moe_models = _get_supported_moe_models()
    if not supported_moe_models or not isinstance(model, supported_moe_models):
        return

    outer_model = model
    inner_model = getattr(outer_model, "model", None) or getattr(outer_model, "language_model", None)
    if inner_model is None:
        raise ValueError("The provided model does not have a valid 'model' or 'language_model' attribute.")

    mixtral_model_cls = _get_mixtral_model_cls()
    for layer in inner_model.layers:
        mlp_attr = "block_sparse_moe" if mixtral_model_cls is not None and isinstance(outer_model, mixtral_model_cls) else DEFAULT_MLP_ATTR
...
class VLLMHijack:
    @staticmethod
    def hijack():
        LoRAModel, _, get_adapter_absolute_path, LRUCacheWorkerLoRAManager = _get_lora_runtime()

        def hijack__load_adapter(self, lora_request) -> LoRAModel:
            """
            ...
            try:
                ...
                if _is_tensor_lora_request(lora_request):
