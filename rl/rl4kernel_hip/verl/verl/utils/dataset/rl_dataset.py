...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        # Explicit control for tokenizer.apply_chat_template(..., enable_thinking=...).
        # Set to None to avoid passing this argument.
        ...
        tools_kwargs_builder_cfg = config.get("tools_kwargs_builder", None) or {}
        self.tools_kwargs_builder = load_extern_type(
            tools_kwargs_builder_cfg.get("path"),
            tools_kwargs_builder_cfg.get("name"),
        )
        ...
        if self.filter_overlong_prompts:
            ...
            if processor is not None:
                ...
                def doc2len(doc) -> int:
                    ...
                    raw_prompt = self._apply_chat_template(
                        self.processor,
                        messages,
    ...
    def _apply_chat_template(self, processing_class, messages, add_generation_prompt: bool, tokenize: bool, return_dict: bool = False):
        kwargs = {
        ...
        try:
            ...
            # Backward compatibility: some tokenizers/processors may not support enable_thinking.
    ...
    def __getitem__(self, item):
        ...
        if self.tools_kwargs_builder is not None:
            try:
                built_tools_kwargs = self.tools_kwargs_builder(copy.deepcopy(row_dict))
                if built_tools_kwargs:
                    tools_kwargs = built_tools_kwargs
            except Exception as exc:
                logger.warning("tools_kwargs_builder failed for index {}: {}", index, exc)
