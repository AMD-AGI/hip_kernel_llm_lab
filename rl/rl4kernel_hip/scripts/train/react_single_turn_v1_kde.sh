...
export KERNEL_NOVELTY_LOG_ALL="1"  # "1"=记录全量, "0"=只记录复制/异常case
...
#############################################
# Data & Model
TRAIN_FILE="./dataset/kernel-agent-single-sft-1125/verl_parquet/rl_data_hard_normal_mixed_mi300x_react_sample_json_v1_verl.parquet"
VAL_FILE="./dataset/kernel-agent-single-sft-1125/verl_parquet/rl_data_hard_normal_mixed_mi300x_react_sample_json_v1_verl.parquet"
MODEL_PATH="./models/Qwen3-8B"  

RES_LENGTH=16384           #24576      #bkup: 32576
PROMPT_LENGTH=4096    #2048: prompt clip ratio - 25%
...
# Rollout & Sampling
ROLLOUT_N=8 
TEMPERATURE=1.0          # 提高采样温度 (0.6→1.0) 增加输出随机性
VAL_N=4
VAL_DO_SAMPLE=True
TOP_P=0.95               # 添加 nucleus sampling
# TOP_k=-1

# Entropy & KL Regularization (增加探索性)
ENTROPY_COEFF=0.015       # entropy bonus: 鼓励模型保持输出多样性
USE_KL_LOSS=False         # 启用KL损失
KL_LOSS_COEF=0.0        # KL系数: 防止策略过快偏离参考模型
...
# Resume Load Tuning
# Only throttle actor optimizer restore in phase 1: it is the largest checkpoint payload,
# and we deliberately avoid changing model-load semantics until this path is validated.
LOAD_USE_MMAP=True
...
# In hybrid actor+vLLM mode we prefer dummy_dtensor so rollout init does not eagerly
# load a second full base model; actor weights are synced into vLLM before generation.
ROLLOUT_LOAD_FORMAT="dummy_dtensor"
# This trades GPU memory for lower CPU RSS. Flip back to True if GPU memory becomes the bottleneck.
ACTOR_OPTIMIZER_OFFLOAD=False

# vLLM & Memory
# On ROCm, Ray's node-memory monitor can count vLLM's large HBM/KV-cache reservation
# toward process RSS. 0.85 is too aggressive for 8x MI325X here and can kill workers
# during post-resume rollout init even when true host DRAM is fine.
GPU_MEMORY_UTILIZATION=0.6
ENABLE_CHUNKED_PREFILL=True
# With chunked prefill enabled, the spmd vLLM path requires this to be >= prompt + response length.
MAX_NUM_BATCHED_TOKENS=$((PROMPT_LENGTH + RES_LENGTH))

# Ray / ROCm Memory Monitor
# Ray's node-memory OOM killer can misclassify ROCm GPU-backed mappings as host memory
# during vLLM rollout init. Keep the monitor on, but raise the threshold by default.
...
PG_LOSS_AGG_MODE="seq-mean-token-sum-norm"     # "token-mean"
# KDE log-odds advantage knobs (CDF bounds: [eps_l, 1-eps_u])
KDE_EPS_L=0.01
KDE_EPS_U=0.01
KDE_TAU=1.0
KDE_ALPHA=2.0             # phi(u) = alpha*log(u) - beta*log(1-u)
KDE_BETA=1.0
...
REWARD_NAME="compute_score"           # 单样本函数（fallback用）
REWARD_NAME_BATCH="compute_score_batch"
REWARD_MANAGER="batch_parallel"       ##dapo or batch_parallel
REWARD_MODE="correct_speedup_copy_penalty"  #soft_clip_novelty
REWARD_CORRECT_SPEEDUP_R_OK=0.3
REWARD_CORRECT_SPEEDUP_CAP=10
REWARD_CORRECT_SPEEDUP_COPY_REWARD=0.0

# Server URL配置（注意：批量模式会自动转换为/run_code_batch）
# - 单样本模式: 直接使用 http://localhost:8080/run_code
# - 批量模式: 自动转换为 http://localhost:8080/run_code_batch
# export SF_URL="http://localhost:8080/run_code"  #local server
export SF_URL="http://172.16.70.144:8080/run_code"
...
EXPERIMENT_NAME="8b-16k-grpo-kde-react-single-turn-Exp02"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --kde-eps-l)   KDE_EPS_L="$2"; shift 2 ;;
    --kde-eps-u)   KDE_EPS_U="$2"; shift 2 ;;
    --kde-tau)     KDE_TAU="$2"; shift 2 ;;
    --kde-alpha)   KDE_ALPHA="$2"; shift 2 ;;
    --kde-beta)    KDE_BETA="$2"; shift 2 ;;
...
export HIP_KERNEL_TOOL_SERVER_URL="${HIP_KERNEL_TOOL_SERVER_URL:-${SF_URL%/run_code}}"
...
echo "[INFO] KDE_EPS_L=${KDE_EPS_L}"
echo "[INFO] KDE_EPS_U=${KDE_EPS_U}"
echo "[INFO] KDE_TAU=${KDE_TAU}"
echo "[INFO] KDE_ALPHA=${KDE_ALPHA}"
echo "[INFO] KDE_BETA=${KDE_BETA}"
...
python3 -m verl.trainer.main_ppo \
  algorithm.adv_estimator=kde_logodds \
  algorithm.kde_eps_l=$KDE_EPS_L \
  algorithm.kde_eps_u=$KDE_EPS_U \
  algorithm.kde_tau=$KDE_TAU \
  algorithm.kde_alpha=$KDE_ALPHA \
  algorithm.kde_beta=$KDE_BETA \
  \
