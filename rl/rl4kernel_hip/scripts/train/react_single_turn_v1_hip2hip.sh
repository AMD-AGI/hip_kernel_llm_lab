...
# Kernel Novelty Tracking is configured after EXPERIMENT_NAME is set.
...
#############################################
# Data & Model
TRAIN_FILE="./dataset/hip2hip_parquet/rl_data_hard3k_normal10k_mixed_hip2hip_mi300x_react_sample_json_v1_verl.parquet"
VAL_FILE="./dataset/hip2hip_parquet/rl_data_hard3k_normal10k_mixed_hip2hip_mi300x_react_sample_json_v1_verl.parquet"
MODEL_PATH="./models/Qwen3-8B"  

RES_LENGTH=16384           # full-file HIP outputs need more budget than kernel snippets
PROMPT_LENGTH=8192         # full-file starter prompts are longer than kernel2kernel prompts
...
ENTROPY_COEFF=0.025       # entropy bonus: 鼓励模型保持输出多样性
...
export SF_URL="http://172.16.41.175:8080/run_code"              # 172.16.41.175; 172.16.70.144
...
EXPERIMENT_NAME="8b-16k-hip2hip-grpo-vanilla-react-single-turn-Exp02"
export KERNEL_NOVELTY_LOG_DIR="./kernel_novelty_logs/${EXPERIMENT_NAME}"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --prompt-length) PROMPT_LENGTH="$2"; MAX_NUM_BATCHED_TOKENS=$((PROMPT_LENGTH + RES_LENGTH)); shift 2 ;;
    --res-length)    RES_LENGTH="$2"; MAX_NUM_BATCHED_TOKENS=$((PROMPT_LENGTH + RES_LENGTH)); shift 2 ;;
    --experiment-name) EXPERIMENT_NAME="$2"; export KERNEL_NOVELTY_LOG_DIR="./kernel_novelty_logs/${EXPERIMENT_NAME}"; shift 2 ;;
